...
export class EditConditionPage {
  ...
  constructor(page: Page) {
    ...
    this.pageHeading = page.getByRole('heading', {
      name: /Create new condition/i
    })
    this.selectQuestionDropdown = page.locator(
      'select[name="items[0][componentId]"]'
    )
    this.selectQuestionButton = page.locator(
      'button[name="action"][value="confirmSelectComponentId"]'
    )
    this.operatorDropdown = page.locator('select[name="items[0][operator]"]')
    this.selectOperatorButton = page.locator(
      'button[name="action"][value="confirmSelectOperator"]'
    )
    this.addAnotherConditionButton = page.getByRole('button', {
      name: /Add another condition/i
    })
    this.conditionNameInput = page.locator('#displayName')
    this.saveConditionButton = page.getByRole('button', {
      name: /Save condition/i
    })
  ...
  async selectYesNoValue(value: 'Yes' | 'No') {
    ...
    await this.page
      .locator(
        `input[type="radio"][name="items[0][value]"][value="${radioValue}"]`
      )
      .check()
  }
}
