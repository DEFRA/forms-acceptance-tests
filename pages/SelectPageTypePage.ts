...
export class SelectPageTypePage {
  ...
  async choosePageType(pageType: 'question' | 'guidance') {
    const radioButton =
      pageType === 'question' ? this.questionPageRadio : this.guidancePageRadio
  ...
  async verifyPageStructure() {
    ...
    await expect(this.questionPageHint).toHaveText(
      'A page to hold one or more related questions'
    )
    await expect(this.guidancePageLabel).toBeVisible()
    await expect(this.guidancePageLabel).toHaveText('Guidance page')
    await expect(this.guidancePageHint).toHaveText(
      'If you need to add guidance without asking a question'
    )
  }
}
