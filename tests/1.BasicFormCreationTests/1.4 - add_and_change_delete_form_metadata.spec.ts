...
test('1.4.1 - should add an email address for form submissions', async ({
  page
}) => {
  // create a form
  ...
  expect(
    (await formPage.successBannerEmailUpdatedIsDisplayed()).isVisible
  ).toBeTruthy()
  expect(await page.getByText(submissionsEmail).isVisible()).toBeTruthy()
})

test('1.4.2 - should update email address for form submissions', async ({
  page
}) => {
  // create a form
  const formPage = new FormPage(page)
  ...
  await expect(page.getByText('new-admin@test.gov.uk')).toBeVisible({
    timeout: 10000
  })
})
...
test('1.4.3 - should delete an existing draft form', async ({ page }) => {
  ...
  const searchResult = await page.getByRole('heading', {
    name: 'There are no matching forms'
  })
...
test('1.4.6 - should add email address for support', async ({ page }) => {
  ...
  await page
    ...
    .fill('We aim to respond within 10 working days')

  await formPage.saveAndContinueButton.click()
  await page.waitForLoadState('networkidle')
  await expect(page.getByText(supportEmail)).toBeVisible()
  await expect(
    page.getByText('We aim to respond within 10 working days')
  ).toBeVisible()
})
...
test('1.4.8 - should add online contact link for support', async ({ page }) => {
  ...
  const textToDescribeTheContact = page.getByRole('textbox', {
    name: 'Text to describe the contact'
  })
...
test('1.4.9 - should add what happens next', async ({ page }) => {
  ...
  const whatHappensNextText =
    'You will receive a confirmation email and next steps.'
  await formPage.enterWhatHappensNextLink.click()
  const whatHappensNextInput = page.getByRole('textbox', {
    name: 'What will happen after a user'
  })
...
test('1.4.10 - should update what happens next', async ({ page }) => {
  ...
  const updatedWhatHappensNextText =
    'You will be contacted by our team for further instructions.'
...
test('1.4.11 - should add a privacy notice URL for the form', async ({
  page
}) => {
  // Create a form
  const formPage = new FormPage(page)
...
test('1.4.12 - should add a privacy notice markdown content for the form', async ({
  page
}) => {
  // Create a form
  const formPage = new FormPage(page)
