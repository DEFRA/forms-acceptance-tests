...
test.describe('@accessibility', () => {
  test.skip('2.2.1 - Forms Runner Accessibility', async ({ page }, testInfo) => {
    ...
    await page.getByRole('textbox', { name: "What's your name?" }).fill('BOB the builder')
    ...
    await page.getByRole('textbox', { name: "What's your email address?" }).fill('test@defra.gov.uk')
    await page.getByRole('button', { name: 'Continue' }).click()

    await page.getByRole('textbox', { name: "What's your phone number?" }).fill('123654789')
