...
test('should create a condition for Yes/No and apply it to page 2', async ({ page }) => {
    ...
    await formPage.goTo();
    const formName = 'Condition test form ' + Math.random().toString().substring(2, 8);
    ...
    await expect(page.getByRole('columnheader', { name: /Used in/i })).toBeVisible();
...
test('should not allow creating two conditions with the same name', async ({ page }) => {
    ...
    const pageOverview = new PageOverview(page);
    await formPage.goTo();
    const formName = 'Duplicate condition test ' + Math.random().toString().substring(2, 8);
    await formPage.enterFormName(formName);
    await formPage.selectRadioOption('Environment Agency');
    await formPage.fillTeamDetails('Team A', 'test@test.gov.uk');
    await formPage.editDraft();
