...
test.skip('3.2.1 - Download form as JSON (both ways)', async ({ page }) => {
  ...
  const json1 = await downloadAndParseJson(page, () =>
    addEditPagesPage.clickDownloadForm()
  )

  // Method 2: Download from Upload a form page
  await addEditPagesPage.clickUploadForm()
  await uploadPage.verifyMainHeading()
  const json2 = await downloadAndParseJson(page, () =>
    uploadPage.clickDownloadCopyLink()
  )
...
test.skip('3.2.2 - Upload a form JSON file and verify', async ({ page }) => {
  ...
  const downloadedJson = await downloadAndParseJson(page, () =>
    addEditPagesPage.clickDownloadForm()
  )
