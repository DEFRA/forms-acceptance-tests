...
test('3.3.1 - should error when adding a page with a duplicate name', async ({ page }, testInfo) => {
  ...
  await formPage.addNewPageButton.click();
  await selectPageTypePage.choosePageType('question');
  await formPage.addNewQuestionPage('What is your name?', 'Your name is being duplicated');
